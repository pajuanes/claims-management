...
async def get_claims():
    ...
    claims_data = execute_query(claims_query)
    ...
    for claim_row in claims_data:
        ...
        damages_data = execute_query(damages_query, (claim_id,))
...
async def get_claim(claim_id: int):
    ...
    claim_data = execute_one(claim_query, (claim_id,))
...
async def create_claim(claim: ClaimCreate):
    ...
    """
    result = execute_one(query, (claim.title, claim.description, claim.status))
...
async def update_claim_status(claim_id: int, payload: ClaimStatusUpdate):
    ...
    row = execute_one("SELECT id, title, description, status FROM claims WHERE id = %s", (claim_id,))
    if not row:
        raise HTTPException(status_code=404, detail="Claim not found")
